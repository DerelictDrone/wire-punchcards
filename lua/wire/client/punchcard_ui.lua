...
local function createPunchCardUIElement(keep)
	local UI = (keep and Wire_PunchCardUI) or vgui.Create("DFrame",nil,"PunchCardUI") -- Fails in MP on first tick, maybe custom elements aren't loaded yet.
	return UI
end
...
if not Wire_PunchCardUI_LoadHook then
	...
	function Wire_PunchCardUI_LoadHook(fn,n)
