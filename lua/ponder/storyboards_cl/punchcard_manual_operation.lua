local function load()
    local storyboard = Ponder.API.NewStoryboard("Wire_PunchCard", "Punchcards", "punchcard-operation")
    storyboard:WithMenuName("wire_punchcard.ponder.card.operation.menuname")
    storyboard:WithPlaybackName("wire_punchcard.ponder.card.operation.playname")
    storyboard:WithModelIcon("models/punch_card/punch_card.mdl")
    storyboard:WithDescription("wire_punchcard.ponder.card.operation.description")
    storyboard:WithIndexOrder(4)
    local chapter1 = storyboard:Chapter()
    chapter1:AddInstruction("PlaceModel", {
        Name  = "Punchcard",
        IdentifyAs = "wire_punchcard.ponder.card.operation.punchcard_inspect",
    ...
    chapter2:AddInstruction("ShowText", {
        Name = "punchcard_txt",
        Text = "wire_punchcard.ponder.card.operation.punchcard_text_1",
        Time = 0,
        Position = Vector(0, 0, 12.5),
    })
    ...
    chapter2:AddInstruction("ChangeText", {
        Name = "punchcard_txt",
        Text = string.format(language.GetPhrase("wire_punchcard.ponder.card.operation.punchcard_text_2"),string.upper(input.LookupBinding("+walk")),string.upper(input.LookupBinding("+use"))),
    })
    chapter2:AddInstruction("Delay", {Time = 0, Length = 4})
    ...
    chapter3:AddInstruction("ChangeText", {
        Name = "punchcard_txt2",
        Text = string.format("wire_punchcard.ponder.card.operation.punchcard_text_4"),
    })
    local chapter4 = storyboard:Chapter()
    chapter4:AddInstruction("Delay", {Time = 0, Length = 4})
    ...
    chapter4:AddInstruction("ChangeText", {
        Name = "punchcard_txt2",
        Text = string.format("wire_punchcard.ponder.card.operation.punchcard_text_5"),
    })
    chapter4:AddInstruction("Delay", {Time = 0, Length = 4})
    ...
    chapter4:AddInstruction("Delay", {Time = 0, Length = 6})
end
